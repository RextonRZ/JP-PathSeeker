...
public class CourseListFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new CourseAdapter(requireContext(), courseList, this::onCourseSelected);
    ...
    private void filterCourses(String query) {
        ...
        adapter = new CourseAdapter(requireContext(), filteredList, this::onCourseSelected);
        recyclerView.setAdapter(adapter);
    }

    private void onCourseSelected(Course course) {
        try {
            Log.d(TAG, "Course selected: " + course.getCourseName());
            CourseDetailsFragment fragment = CourseDetailsFragment.newInstance(course);

            if (getActivity() instanceof Test) {
                Test activity = (Test) getActivity();
                Log.d(TAG, "Loading fragment in Test activity");
                activity.loadFragment(fragment, true);
            } else {
                Log.e(TAG, "Activity is not Test instance");
            }
        } catch (Exception e) {
